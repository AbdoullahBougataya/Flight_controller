...
void loop(){
  ...
  if(rslt == 0){
    ...
    phiHat_rad = COMP_FLTR_ALPHA * atanf(filteredAccelGyro[4] / filteredAccelGyro[5]) + (1.0f - COMP_FLTR_ALPHA) * (phiHat_rad + dt * (filteredAccelGyro[0] + tanf(thetaHat_rad) * (sinf(phiHat_rad) * filteredAccelGyro[1] + cosf(phiHat_rad) * filteredAccelGyro[2]))); // Roll estimate
